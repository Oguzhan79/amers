...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    let dot = TICK_N % 2 === 0 ? 'Zuid-Kust' : 'Roleplay';
    let embed = new Discord.RichEmbed()
    .setAuthor("Zuid-Kust Server Status", "https://i.imgur.com/id49Sqd.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ Zuid-Kust RolePlay' : '🔵 Zuid-Kust RolePlay')
    .setTimestamp(new Date())
    .addField('\n\u200b\nHoe kan je de server joinen?','Je kan de server joinen doormiddel van **zuidkust.vibegames.pro** in te voeren bij Direct Connect. Onderaan staat de server status om te kijken hoeveel mensen er online zijn en in de wachtrij staan.',false)
    if (STATUS !== undefined)
    {
  ...
  bot.on('ready',() => {
    ...
    // bot.user.setGame('Roofstad', 'https://www.twitch.tv/prxjecs');
    bot.user.setActivity('Zuid-Kust RolePlay',{'url':'https://www.twitch.tv/prxjecs','type':'STREAMING'});
  ...
  bot.on('message', async function (msg) {
    if (msg.channel.id === '586631869928308743') {
        await msg.react(bot.emojis.get('732194699515002911'));
        await msg.react(bot.emojis.get('732195491772891237'));
    }
});
